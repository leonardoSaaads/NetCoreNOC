...
from opticorr import audit, auth, known_oids, severity
from opticorr.api import QuietServer, create_app
from opticorr.correlate import Correlator, ScoredLink, WindowAlarm
from opticorr.events import Fingerprint, QuarantinedPacket, TrapEvent
from opticorr.learn import STORM_ALARMS, STORM_DAMPING, Learner
from opticorr.logsetup import configure_logging
from opticorr.receiver import MAX_INSTANCE_CHARS, QueueItem, start_receiver
...
from opticorr.varbind_profile import MAX_ENTITIES_PER_NE, VarbindProfiler

log = logging.getLogger("opticorr")
...
class Settings:
    ...
    def from_env(cls) -> Settings:
        env = os.environ.get
        return cls(
            db_path=env("OPTICORR_DB", cls.db_path),
            trap_host=env("OPTICORR_TRAP_HOST", cls.trap_host),
            trap_port=int(env("OPTICORR_TRAP_PORT", str(cls.trap_port))),
            http_host=env("OPTICORR_HTTP_HOST", cls.http_host),
            http_port=int(env("OPTICORR_HTTP_PORT", str(cls.http_port))),
            allowlist=env("OPTICORR_ALLOWLIST", cls.allowlist),
            api_token=env("OPTICORR_API_TOKEN", cls.api_token),
            retention_days=float(env("OPTICORR_RETENTION_DAYS", str(cls.retention_days))),
            audit_retention_days=float(
                env("OPTICORR_AUDIT_RETENTION_DAYS", str(cls.audit_retention_days))
            ),
            tls_cert=env("OPTICORR_TLS_CERT", cls.tls_cert),
            tls_key=env("OPTICORR_TLS_KEY", cls.tls_key),
            log_json=env("OPTICORR_LOG_JSON", "") not in ("", "0", "false", "False"),
        )
...
def operator_warnings(allowlist: str, tls_enabled: bool, http_host: str) -> list[str]:
    ...
    if not tls_enabled and http_host not in LOOPBACK_HOSTS:
        warns.append(
            "HTTP is not using TLS on a non-loopback bind. Set OPTICORR_TLS_CERT/KEY or "
            "front OptiCorr with a TLS reverse proxy."
        )
    return warns
...
def _print_bootstrap_banner(password: str) -> None:
    ...
    print("  OptiCorr bootstrap admin created (first run)", flush=True)  # noqa: T201
...
class LegacyTokenRemovedError(RuntimeError):
    """OPTICORR_API_TOKEN was removed in v0.3.0 (§5.8); setting it is now a hard error."""
...
async def run(settings: Settings) -> None:
    if settings.api_token:
        ...
        raise LegacyTokenRemovedError(
            "OPTICORR_API_TOKEN was removed in v0.3.0. Unset it and issue a named service "
            "token instead (admin -> Tokens in the UI, or POST /api/tokens); send it as "
            "'Authorization: Bearer <value>'. See MIGRATION.md."
        )
    ...
    tasks = [
        ...
        asyncio.create_task(engine.maintenance_loop(lambda: runtime.retention_days)),
