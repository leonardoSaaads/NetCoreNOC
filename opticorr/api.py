...
from opticorr import __version__, audit, auth, rbac
...
# Denied (403) sensitive actions audited with this representative catalog action.
DENIED_ACTION = {
...
class QuietServer(uvicorn.Server):
    """Uvicorn server that leaves signal handling to the OptiCorr process."""
...
def create_app(
    ...
    app = FastAPI(title="OptiCorr", version=__version__, docs_url=None, redoc_url=None)
    ...
    def csrf_ok(request: Request) -> bool:
        ...
        if urlsplit(origin).netloc != request.headers.get("host"):
            return False
        return request.headers.get("x-opticorr-client") == "ui"
    ...
    async def security(request: Request) -> auth.Principal:
        ...
        if permission is None or not rbac.role_allows(principal.role, permission):
            if permission in DENIED_ACTION:
                async with store.lock:
                    await audit_row(
                        request,
                        principal,
    ...
    async def graph() -> dict[str, Any]:
        async with store.lock:
            return await store.graph_snapshot(min_edge_n=MIN_EDGE_N)
    ...
    @app.get("/api/situations/{sid}", dependencies=guarded)
    async def situation(sid: int) -> dict[str, Any]:
        async with store.lock:
    ...
    async def timeline(limit: int = 300) -> dict[str, Any]:
        async with store.lock:
            return {"marks": await store.timeline_marks(min(max(limit, 1), 1000))}

    # -- entity tree + varbind profiler (viewer+, inspectable) -------------------------

    @app.get("/api/entities", dependencies=guarded)
    ...
    @app.get("/api/entities/{ne_id}", dependencies=guarded)
    async def entity_detail(ne_id: int) -> dict[str, Any]:
        async with store.lock:
        ...
        return {"ne": ne, "entities": entities_rows, "profiles": profiles, "candidates": candidates}
    ...
    async def events(principal: auth.Principal = Depends(security)) -> StreamingResponse:
        async def snapshot() -> str:
            ...
            payload = {"stats": stats_out, "graph": graph_out, "situations": sits}
