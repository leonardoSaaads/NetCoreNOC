...
class Store:
    ...
    async def create_situation(self, ts: float) -> int:
        cur = await self.conn.execute(
            "INSERT INTO situation (created_at, updated_at) VALUES (?, ?) RETURNING id", (ts, ts)
        )
        row = await cur.fetchone()
        assert row is not None
        return int(row[0])
    ...
    async def set_user_disabled(self, user_id: int, disabled: bool, now: float) -> None:
        await self.conn.execute(
            "UPDATE user SET disabled=?, updated_at=? WHERE id=?", (int(disabled), now, user_id)
        )
