...
> which is this document's own opening argument rather than a later objection to it. The path is
> unchanged because gates, security reviews and build reports cite it by name and are records that
> are never rewritten; the alternative (rename to match, per DECISIONS #93) is a ROADMAP line.
>
