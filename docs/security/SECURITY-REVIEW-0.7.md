...
### Correction to F32 — added 2026-07-29 (v0.7.1)

**Superseded in place, not rewritten.** The F32 row above is left exactly as it was published so
that what this review claimed, and when, stays answerable. Two of its sentences were wrong, and
both were wrong in the same direction — they described the *read* paths and were read as describing
the perimeter.

1. **"One filter applied to every NE-bearing read (list, detail, graph, timeline, entities, stats,
   SSE)."** Accurate, and beside the point. Scope was resolved on every NE-bearing **read** and on
   **none** of the three `editor`-level **write** routes. A scoped editor could write feedback on,
   close, and label network elements they could not see, and the resulting 200-vs-404 split was an
   existence oracle over exactly the resources this row claims are indistinguishable. Corrected by
   **F34** (`SECURITY-REVIEW-0.7.1.md`).

2. **"Enumerating aggregates computed over the in-scope set only."** True of `/api/stats`, which is
   computed by `scoped_stats`. **False** of the truncated list endpoints: `list_situations` and
   `timeline_marks` applied `LIMIT` over the **global** ordering and filtered afterwards, so a
   scoped principal's returned count varied with out-of-scope volume — the aggregate oracle this row
   says is closed. Corrected by **F38**.

A third defect sat underneath both: the scope resolver read the **operator label**, which
`POST /api/labels` writes at `editor` level, so the constrained role was an author of its own scope.
That is **F35**, and it is the reason v0.7.1 states the general rule this review lacked —
*authorization never reads data the constrained party can write.*

Nothing in §4 below is retracted; every residual risk it records is still real. But §4 was written
about a perimeter that was enforced on one side only, so read it together with
`SECURITY-REVIEW-0.7.1.md` §4, which says what this review missed and what changed in the review
method so that the next one does not repeat it.
...
| Existence oracle via a scoped resource | F32; **reads only — writes corrected by F34** | `test_f32_out_of_scope_detail_is_indistinguishable_from_nonexistent`, `test_f34_scope_is_enforced_on_the_editor_write_routes` |
| Leakage through aggregates | F32; **truncated lists corrected by F38** | `test_f32_aggregates_are_computed_over_the_in_scope_set`, `test_f38_truncation_is_applied_after_the_scope_filter` |
